...
const yenEstimate = J.currentEstimateYen || J.dailyEstimateYen || J.grossEstimateYen || {low: null, base: null, high: null};
const yenText = yenEstimate.base == null ? '未更新' : `約¥${yenEstimate.base.toFixed(2)}億`;
const yenRange = yenEstimate.low == null ? '推定レンジ 未更新' : `推定レンジ ¥${yenEstimate.low.toFixed(2)}〜¥${yenEstimate.high.toFixed(2)}億`;
const jpSnapshot = J.snapshotTime ? `${J.date} ${J.snapshotTime}時点` : `${J.date}・${J.updatedAt}更新`;
const jpSeatCaption = J.seatOccupancy != null ? `${J.seatOccupancy.toFixed(1)}%販売` : (J.seatProgress != null ? `${J.seatProgress.toFixed(1)}%消化` : '取得座席');
const jpCoverage = J.referenceCoverage ?? J.seatPlan?.coverage;
const jpCoverageText = jpCoverage == null ? '未更新' : `${jpCoverage.toFixed(1)}%`;
const jpCoverageSub = J.seatPlan ? `${J.seatPlan.trackedTheaters}/${J.seatPlan.allTheaters}館・${J.seatPlan.date}座席計画` : (J.nextDayTrackedTheaters ? `${J.nextDayTrackedTheaters}/${J.nextDayAllTheaters}館` : '取得館率ベース');
const jpOfficialNote = J.officialGrossUsd ? `公式日本累計は${money(J.officialGrossUsd)}。` : '公式興収ではなく、世界累計には加算しません。';
...
const J = D.japanFlash;
const numberJa = value => Number(value).toLocaleString('ja-JP');
const yenEstimate = J.dailyEstimateYen || J.grossEstimateYen || {low: null, base: null, high: null};
const yenText = yenEstimate.base == null ? '未更新' : `約¥${yenEstimate.base.toFixed(2)}億`;
const yenRange = yenEstimate.low == null ? '推定レンジ 未更新' : `推定レンジ ¥${yenEstimate.low.toFixed(2)}〜¥${yenEstimate.high.toFixed(2)}億`;
const jpSnapshot = J.snapshotTime ? `${J.date} ${J.snapshotTime}時点` : `${J.date}・${J.updatedAt}更新`;
const jpSeatCaption = J.seatOccupancy != null ? `${J.seatOccupancy.toFixed(1)}%販売` : (J.seatProgress != null ? `${J.seatProgress.toFixed(1)}%消化` : '取得座席');
...
const jpCoverageSub = J.seatPlan ? `${J.seatPlan.trackedTheaters}/${J.seatPlan.allTheaters}館・${J.seatPlan.date}座席計画` : (J.nextDayTrackedTheaters ? `${J.nextDayTrackedTheaters}/${J.nextDayAllTheaters}館` : '取得館率ベース');
const jpOfficialNote = J.officialGrossUsd ? `公式日本累計は${money(J.officialGrossUsd)}。` : '公式興収ではなく、世界累計には加算しません。';
document.querySelector('#japan-flash').innerHTML = `<div class="japan-flash-lead"><div><span class="flash-live">● ${J.sourceScope || '販売速報'}</span><b>${jpSnapshot}</b></div><strong>日割推定 ${yenText}</strong><small>${yenRange}</small></div><div class="japan-flash-stats"><div><small>取得館販売数</small><strong>${numberJa(J.trackedSales)}</strong><span>${J.status || 'チケット販売速報'}</span></div><div><small>デイリー順位</small><strong>${J.rank}位</strong><span>全作品中</span></div><div><small>取得座席・販売率</small><strong>${numberJa(J.seats)}</strong><span>${jpSeatCaption}</span></div><div><small>推定最終販売 / 取得率</small><strong>${J.estimatedFullDaySales ? numberJa(J.estimatedFullDaySales) : jpCoverageText}</strong><span>${J.estimatedFullDaySales ? `取得率 ${jpCoverageText}` : jpCoverageSub}</span></div></div><p class="japan-flash-note"><b>推定値です。</b>${J.method} ${jpOfficialNote}</p>`;
const billion = v => `$${(v / 1000).toFixed(2)}B`;
document.querySelector('#forecast-card').innerHTML = `<div class="forecast-layout"><div class="forecast-main"><small>中心見込み</small><strong>${billion(D.forecast.base)}</strong><span>${D.forecast.confidence}・モデル推定</span></div><div class="forecast-range"><div class="forecast-labels"><span>下限 ${billion(D.forecast.low)}</span><span>上限 ${billion(D.forecast.high)}</span></div><div class="forecast-track"><i class="forecast-marker"></i></div><div class="forecast-basis">${D.forecast.basis}</div></div></div>`;
